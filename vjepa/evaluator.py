...
class VJEPAEvaluator(BaseEvaluator):
    ...
    def predict_and_get_logits(
        ...
        B = input_ids.shape[0]
        WINDOW_SIZE = 16  # Standard for 1X challenge
        H = W = 16  # Fixed spatial dimensions for 256 tokens (16x16)
        inputs_THW = rearrange(
            input_ids, "b (t h w) -> b t h w", t=WINDOW_SIZE, h=H, w=W
        ).to(self.device)
        ...
        with torch.no_grad():
            ...
            for timestep in range(1, WINDOW_SIZE):
                ...
                frame_logits_1 = frame_logits_1.view(B, H, W, 512).permute(
                    0, 3, 1, 2
                )  # [B, 512, H, W]
                frame_logits_2 = frame_logits_2.view(B, H, W, 512).permute(
                    0, 3, 1, 2
                )  # [B, 512, H, W]
                factored_logits = torch.stack(
                    [frame_logits_1, frame_logits_2], dim=2
                )  # [B, 512, 2, H, W]
                ...
                factored_pred = torch.stack(
                    [pred_tokens_1, pred_tokens_2], dim=-1
                )  # [B, H, W, 2]
                predicted_tokens = unfactorize_token_ids(
                    factored_pred, 2, 512
                )  # [B, H, W]
